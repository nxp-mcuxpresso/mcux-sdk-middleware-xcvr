...
dbgRamStatus_t dbg_ram_init(const dbg_ram_capture_config_t *dbg_ram_configuration)
{
    ...
    else
    {
            ...
            ((stop_sys_ram % 4U) == 0U))
        {
             ...
             */
            uint32_t start_dbg_ram; /* Start offset within the combined TX and RX packet RAM space,  starting from zero
                                       and counting words */
            uint32_t stop_dbg_ram;  /* Stop offset within the combined TX and RX packet RAM space,  starting from zero
                                       and counting words */
