...
typedef struct {
    ...
    uint8_t t_dt0_usec;                 /*!< Mode0 T_DT (sync packet) duration in us ( first DT_TX only. DT_RX_SYNC duration can't be known )*/
...
typedef struct {
    ...
    bool rx_dft_iq_out_averaged;       /*!< True if the selected IQ samples to capture are averaged ( reduced samples rate ) during t_fm and t_pm. */
    bool rsm_dma_mask_used;            /*!< True if t_pm dma mask generated by rsm block is used. False if LCL is providing the mask */
