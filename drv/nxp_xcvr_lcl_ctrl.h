...
#if defined(NXP_RADIO_GEN) && (NXP_RADIO_GEN >= 450) && \
    (RF_OSC_26MHZ == 0) /* RSM only supported on Gen 4.5 or higher radios & with RF_OSC = 32MHz */
...
#define XCVR_RSM_FCS_PKPK_MAX_STEP_COUNT                                                                               \
    (160U) /*!< Maximum number of FCS, Pk-Pk, or Pk-Tn-Tn-Pk steps within any RSM sequence.  This is due to PN and RTT \
              RAM array sizes. */
#else
#define XCVR_RSM_OVERALL_MAX_SEQ_LEN                                                                                   \
    (128U) /*!< Maximum overall sequence length (inclusive) for RSM sequences. No sequence can be longer than this due \
              to Fstep RAM array size. */
#define XCVR_RSM_FCS_PKPK_MAX_STEP_COUNT                                                                               \
    (106U) /*!< Maximum number of FCS, Pk-Pk, or Pk-Tn-Tn-Pk steps within any RSM sequence.  This is due to PN and RTT \
              RAM array sizes. */
#endif
...
{
    XCVR_RSM_RX_MODE =
        0U, /*!< Configure RSM to start with Receive. AKA the SQTE "Reflector" role or the PDE "RD" role */
    XCVR_RSM_TX_MODE =
...
typedef enum
{
    XCVR_RSM_TSM_HPM_CAL_BYP =
        0U, /*!< Configure TSM settings programming to bypass HPM cal  (used only if HPM CAL is completely skipped) */
    XCVR_RSM_TSM_HPM_52US_CAL =
...
{
    XCVR_RSM_SQTE_PN32 =
        0U, /*!< A 32 bit PN sequence is used in SQTE packets. THis setting is programmed in the PHY.  */
    XCVR_RSM_SQTE_PN64 =
 ...
 * averager and it used to generate a single PCT per antenna slot. */
...
typedef struct
{
    ...
    uint8_t rxdig_dly; /*!< Used in longer sequences to optimize enable of the RX digital. Can cause a problem in warmup
    ...
    XCVR_RSM_SQTE_RATE_T rate; /*!< SQTE ONLY: Indicates to the RSM the data rate being used by the XCVR. Must match the
                                  XCVR configuration.  */
    XCVR_RSM_AVG_WIN_LEN_T
        averaging_win;      /*!< The number of IQ or phase samples to average in RXDIG to produce one output sample. */
    bool use_rsm_dma_mask;  /*!< Selects to use the RSM DMA mask signal rather than the LCL DMA mask. Only valid when 1
                               antenna is used. */
    uint8_t rsm_dma_dly_pm; /*!< DMA delay from end of RX warmup to assertion of DMA mask signal in usec. Applies to
                               PM_RX state only. RSM_DMA_DLY field. */
...
#if defined(NXP_RADIO_GEN) && (NXP_RADIO_GEN >= 470) /* Only applies to KW47 */
    uint16_t mode0_timeout_usec; /*!< Timeout (in usec) for RSM in reflector role to wait for a Mode 0 step transmission
                                    from initiator. */
    XCVR_RSM_PCT_AVG_WIN_LEN_T
...
/*! @brief Structure for storing rsm State timings and duration  */
typedef struct
{
    uint16_t t_fc_usec;                 /*!< RSM_T_FC duration in us (FC_RX2TX and FC_TX2RX) */
    uint16_t t_ip1_usec;                /*!< RSM_T_IP1 duration in us (IP1_RX2TX and IP1_TX2RX) */
    uint16_t t_ip2_usec;                /*!< RSM_T_IP2 duration in us (IP2_RX2TX and IP2_TX2RX) */
    uint8_t t_s_usec;                   /*!< RSM_TS duration in us (S_TX2TX and S_RX2RX) */
    uint16_t t_fm_usec[T_FM_FLD_COUNT]; /*!< RSM_T_FM[0..n] durations in us (FM_TX and FM_RX) */
    uint16_t t_pm_usec[T_PM_FLD_COUNT]; /*!< RSM_T_PM[0..n] durations in us (PM_TX and PM_RX) */
    uint8_t t_dt0_usec;    /*!< Mode0 T_DT (sync packet) duration in us ( first DT_TX only. DT_RX_SYNC duration can't be
                              known )*/
    uint16_t t_dt_usec;    /*!< Mode1 and Mode3 T_DT (rtt packet) duration in us (DT_TX and DT_RX) */
    uint16_t warmup_usec;  /*!< WU state duration in us */
    uint8_t warmdown_usec; /*!< WD state duration in us */
} xcvr_lcl_rsmstate_duration_t;

/*! @brief Structure for storing rsm dma mask timings and duration */
typedef struct
{
    uint8_t dma_fm_dur;  /*!< DMA mask duration in us during t_fm */
    uint8_t dma_pm_dur;  /*!< DMA mask duration in us during t_pm */
    uint16_t dma_iq_avg; /*!< IQ sample averaging window. Number of samples averaged ( also sample rate divider ) */
    uint8_t sample_rate_per_usec; /*!< IQ capture number of sample per microseconds */
    bool rx_dft_iq_out_averaged;  /*!< True if the selected IQ samples to capture are averaged ( reduced samples rate )
                                     during t_fm and t_pm. */
    bool rsm_dma_mask_used; /*!< True if t_pm dma mask generated by rsm block is used. False if LCL is providing the
                               mask */
    uint8_t dma_signal_valid_mask_sel; /*!< DMA_SIGNAL_VALID_MASK_SEL field value. Is the dma mask selection */
} xcvr_lcl_rsmdma_config_t;

/* RSM-related register backup structure. Stores registers that must be changed in different peripherals for RSM to
 * work. */
typedef struct
{
...
typedef uint32_t
    tsm_u32_backup_array_t[NUM_TSM_U32_REGISTERS]; /*!< Backup array of uint32_t to store entire TSM register set. */
 ...
 *
 * @note This function pointer type is only available for the NBU CPU as the CM33 CPU does not have connection to RSM
 * interrupt line
 */
...
/*! @brief RSM IRQ enable/disable masks; Intended to be OR'd together to enable or disable multiple interrupts at a
 * time. */
...
#define XCVR_LCL_RSM_IRQ_STAT_ALL_BITS                                                                                 \
    (XCVR_MISC_RSM_CSR_RSM_IRQ_IP1_MASK | XCVR_MISC_RSM_CSR_RSM_IRQ_IP2_MASK | XCVR_MISC_RSM_CSR_RSM_IRQ_FC_MASK |     \
     ...
     XCVR_MISC_RSM_CSR_RSM_IRQ_ABORT_MASK) /* all of the interrupt STATUS bits. Useful also in creating an error check \
                                              mask */
...
#if defined(KW45B41Z82_NBU_SERIES) || \
    defined(KW45B41Z83_NBU_SERIES) /* RSM interrupts are only supported on the NBU CPU */
/*!
 ...
 *
 * @param[in] mask  The OR'd mask of multiple interrupts to enable or disable. The mask must use the RSM CSR enable bits
 * in their proper position from the register header file.
 ...
 *
 * This function validates the Ranging State Machine (RSM) initialization settings structure. It is implemented as a
 * separate function in order to simplify the init routine and also to allow for validation ahead of time in case the
 * init routine is in a critical timing path.
 *
 ...
 *
 * This function initializes any customized PLL settings for RSM operaiton. It is intended to be used to as a helper for
 * both ::XCVR_LCL_RsmInit() and for
 ...
 *
 * @note This routine modifies a number of registers which must be saved and then restored before changing back to
 * normal operation. The ::XCVR_LCL_RsmDeInit() function is intended to return to the prior settings at the end of RSM
 ...
 *
 * This function initializes the Ranging State Machine (RSM), TSM, TX_DIG, GEN4PHY, and other registers before TX or RX
 * operations using the RSM module. This function calls helper functions to implement some of the TSM timing
 *
 * @param rsm_settings_ptr the pointer to a settings structure for RSM initialization.
 *
 * @return The status of the init process.
 ...
 *
 * This function resets the registers touched by ::XCVR_LCL_RsmInit() to allow the radio to return to normal operating
 * modes. After this routine is called, the RSM will be disabled and the TSM, TX_DIG, GEN4PHY will all be returned to
 * prior settings.
 *
 */
 ...
 *
 * This function aborts any ongoing RSM operations prior to the normal end of sequence. It is also used to stop the RSM
 * after normal completion of a RSM sequence.
 *
 * @note This routine performs a wait for the RSM state machine to return to IDLE state to ensure it is safe to clear
 * the abort bit or clear RX or TX enables.
 * @note This routine must be called every time that RSM is started via ::XCVR_LCL_RsmGo() as it restores registers that
 * are reprogrammed by the
 * ::XCVR_LCL_RsmGo() routine to support the RSM. Failure to this will cause the normal Bluetooth or GENFSK radio
 ...
 *
 * This function starts the RSM in the desired (RX or TX) role. If the RSM has a hardware trigger selected then the RSM
 * will start upon the assertion of that trigger. If the RSM has a software trigger selected then the RSM will start
 * immediately. For both hardware and software triggers, the RSM_TRIG_DLY will control the time delay to the actual
 * start of the RSM sequence.
 *
 ...
 *
 * @note Every RSM sequence that is started must be either aborted or stopped in order to prevent unexpected executions
 * of the RSM sequence due to hardware triggers. The function
 ...
 *
 * This function captures the state of the TSM timing registers and is used both before and after calling
 * ::XCVR_LCL_RsmInit(). When called before, it stores the state of the TSM for normal operation. When called after, it
 * stores the state of the TSM for RSM operations. Both cases are intended to allow for later restore of a specific set
 * of TSM timings without performing additional calculations.
 *
 ...
 * DMA.
 *
 * This function captures tor restores he state of the TSM timing registers with a fast DMA block storing to one of the
 ...
 * .
 *
 * This function updates a subset of new TSM timing register values for specific RSM operations. The input structure is
 * the relevant TSM register changes only. It is also used to restore TSM timings from a backup structure (replaces
 * deprecated function XCVR_LCL_SetTsmTimings).
 *
 ...
 *
 * This function restores registers to multiple XCVR blocks to replace their state to what it was before or during RSM
 * operations
 *
 * @param reg_backup_ptr the pointer to a settings structure for register restore.
 *
 ...
 *
 * This function unpacks the raw data structure that was read from RTT Packet RAM and places the data into a formatted
 * structure.
 *
 ...
 *
 * This formats the channel number, CTUNE data, HPM CAL factor, step format and T_PM_SEL values into the Fstep
 * structure.
 *
 ...
 *
 * @param freq_list the pointer to an array of frequencies to tune to and capture calibration values. This must follow
 * the format defined in the PLL's CHAN_MAP[HOP_TBL_CFG_OVRD] bitfield and match the current setting of the field.
 * Frequency values need not be sorted.
 ...
 * @param update_curve_fit selects for updating the curve fit data storage when the calibration frequency is 2442MHz.
 * Error is triggered if 2442MHz is not in the frequency list.
 * @param rate the data rate to be used in RSM operation.
 *
...
/*!
 * @brief Function to calculate interpolated values for HPM CAL based on previously calculated equations for
 * approximations.
 *
 * This function calculates interpolated values for HPM CAL values at different frequencies based on data captured in
 * the XCVR_LCL_CalibratePll() routine when the update_curve_fit parameter is set to true. These equations are generated
 * from a single HPM_CAL measurement at 2442MHz and are used to generate many additional frequency points data without
 * needing additional PLL calibrations.
 *
 * @param hadm_chan_idx_list the pointer to an array of HADM channel index values that correspond to the cal_results
 * array. Values may be in any order.
 * @param cal_results the pointer to an array of structures to store the interpolation results in raw format matching
 * the frequency array.
 ...
 * @note The interpolation assumes data in the curve fit description structure is valid and performs no check on the
 * input data other than NULLPTR check.
 *
 */
 ...
 *
 * This function triggers a RX warmup and a manual DCOC process in the receiver. The result of this process is that the
 * DCOC registers are properly programmed for use in RSM operations.
 *
 ...
 *
 * This function sets up the TSM overrides needed to keep the PLL TX and RX divider states consistent across multiple
 * Channel Sounding events/subevents.
 *
 */
 ...
 *
 * This function releases all the TSM overrides configured in either of ::XCVR_LCL_ContPhaseOvrd() or
 * ::XCVR_LCL_SetupManualDcoc() or ::XCVR_LCL_EnablePhaseMeasure().
 *
 */
 ...
 * called after all phase measurement is complete in order to remove the ADC and DAC overrides that are setup in
XCVR_LCL_SetupManualDcoc().
 *
 * @code
#define RESID_TEST_NUM		(10U)
    ...
    for (count =0;count<RESID_TEST_NUM;count++)
    {
        ...
        lclstatus = XCVR_LCL_ProcessPhaseMeasure(&dc_resid_continuous[count].i_resid,
&dc_resid_continuous[count].q_resid, temp_resid); assert(lclstatus == gXcvrLclStatusSuccess);
    }
    ...
    for (count =0;count<RESID_TEST_NUM;count++)
    {
        ...
        assert(lclstatus == gXcvrLclStatusSuccess);
        temp_resid = XCVR_RX_DIG->DCOC_DIG_CORR_RESULT;
        lclstatus = XCVR_LCL_ProcessPhaseMeasure(&dc_resid_discontinuous[count].i_resid,
&dc_resid_discontinuous[count].q_resid, temp_resid); assert(lclstatus == gXcvrLclStatusSuccess);
    }
    XCVR_LCL_AllPhaseRelease(); // Release all overrides, both measurement and continuous phase enablement
    XCVR_LCL_OverrideDcoc(temp_dcoc_ctrl2_val, FALSE); // Release overrides
    temp_rf_ctrl_val = XCVR_LCL_CalibrateDcocStart(XCVR_RSM_RATE_1MBPS);
 ...
 *
 * This function forces the DCOC_CTRL2 register to a new value and sets the override bits to make these manual values
 * active
 *
 ...
 *
 * This function sets up the TSM overrides needed to prepare for measuring the current PLL phase. It must be called
 * every time before a phase measurement is performed (i.e. before the DCOC calibration)
 *
 * @note
 */
 ...
 * @param dc_resid_val the 32 bit contents of the XCVR_RX_DIG->DCOC_DIG_CORR_RESULT register for processing. This is
 * passed as a parameter to ensure testability of the function.
 *
 * @return the status of the measurement process
 *
 * @note The I and Q residual values are intended to be used to calculate a phase angle as a measurement of the PLL
 * phase. The absolute value is not meaningful but comparison between two different measurements may be used to
 * determine if the PLL has the same or inverted phase (from one measurement to the next). After completion of this
 ...
 *
 * This function takes and input Carrier Frequency Offset compensation in Hz and calculates an offset to apply to the
 * PLL numerator to implement the compensation
 *
 * @param cfo_in_hz the signed CFO to compensate in Hz. Positive CFO value == negative PLL frequency adjustment;
 * Negative CFO value == positive PLL frequency adjustment; Zero == no adjustment.
 *
 * @return The status of the compensation process.
 *
 *
 ...
 *
 * This function returns the Carrier Frequency Offset compensation in Hz based on the value in the PLL_OFFSET_CTRL
 * register.
 *
 * @return The amount of compensation currently programmed, in Hz.
 *
 * @note Due to the use of integer divide in this routine and in ::XCVR_LCL_RsmCompCfo(), the returned value may not be
 * exactly the original requested compensation.
 */
 ...
 *
 * This function maps from a HADM channel index to the channel number value used to program the FSTEP channel number
 * field according to HOP_TBL_CFG_OVRD format #3.
 *
 * @param hadm_chan_index The HADM standard specified channel index, ranging from 0 to 78 to select frequencies of
 * 2402MHz up to 2480MHz.
 ...
 *
 * This function reads the rsm register configuration and iterate over the step settings to compute the total sequence
 * buffer size needed for dma iq capture and the dma sequence length in us ( to be used in antena switch configuration
 * ). The dma buffer size corresponds to the number of iq samples to be captured.
 *
 * @param[in]  fstep_settings frequency step list pointer (xcvr_lcl_fstep_t), the first element of a frequency step
 * configuration array.
 ...
 * @param[out] dma_buffer_size (uint16_t) address to store the dma buffer size corresponding to the current
 * configuration
 * @param[out] dma_seq_length_us (uint16_t) address to store the dma sequence length in us corresponding to the current
 * configuration
 ...
 *
 * @return The count of FCS, Pk-Pk , and Pk-Tn-Tn-Pk steps within the overall frequency step list. A value of 0xFFU is
 ...
 *
 * This function configures PLL dividers under TSM control (rather than static signals keeping dividers running
 * continuously) .
 *
