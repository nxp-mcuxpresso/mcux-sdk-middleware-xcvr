                                   ...
                                   uint8_t * ant_cs_sync)
{
    ...
    *step_format = (XCVR_RSM_FSTEP_TYPE_T)((temp&COM_MODE_013_CFG_HDR_STEP_CFG_MODE_MASK)>>COM_MODE_013_CFG_HDR_STEP_CFG_MODE_SHIFT);
    *t_pm_sel = (XCVR_RSM_T_PM_FM_SEL_T)((temp&COM_MODE_013_CFG_HDR_STEP_CFG_T_PM_SEL_MASK)>>COM_MODE_013_CFG_HDR_STEP_CFG_T_PM_SEL_SHIFT);
    *ant_permut = (temp&COM_MODE_013_CFG_HDR_STEP_CFG_ANT_PERMUT_MASK)>>COM_MODE_013_CFG_HDR_STEP_CFG_ANT_PERMUT_SHIFT;
    ...
    *ant_cs_sync = (temp&COM_MODE_013_CFG_HDR_STEP_CFG_ANT_CS_SYNC_MASK)>>COM_MODE_013_CFG_HDR_STEP_CFG_ANT_CS_SYNC_SHIFT;

    /* Return the length of the header as the function return value */
    XCVR_RSM_RTT_TYPE_T rtt_type = (XCVR_RSM_RTT_TYPE_T)((XCVR_MISC->RSM_CTRL2&XCVR_MISC_RSM_CTRL2_RSM_RTT_TYPE_MASK)>>XCVR_MISC_RSM_CTRL2_RSM_RTT_TYPE_SHIFT);
                ...
                if (temp_status == gXcvrLclStatusSuccess)
                {
                    ...
                    switch (*step_type_ptr)
                    {
                        case XCVR_RSM_STEP_FCS:
                            ...
                            {
                                mode1_3_step_ptr = (xcvr_lcl_mode_0_1_3_cfg_t *)GET_PKT_RAM_PTR(pkt_ram_info_ptr->config_pkt_ram_bank, (pkt_ram_info_ptr->config_base_addr_word)+curr_addr);
                            }
                            else
                            {
                            ...
                            XCVR_LCL_MakeCommonHeader(step_hdr_ptr, *channel_list_ptr,0U, *step_cfo_ptr, *hpm_cal_list_ptr, *phase_add_ptr, *step_cfg_list_ptr);
                            XCVR_LCL_WriteCommonHeader((COM_MODE_CFG_HDR_UNION_Type *)&(mode1_3_step_ptr->header), step_hdr_ptr);
                        ...
                        case XCVR_RSM_STEP_TN_TN:
                            ...
                            if (load_directly_to_pkt_ram)
                            {
                                mode2_step_ptr = (xcvr_lcl_mode_2_cfg_t *)GET_PKT_RAM_PTR(pkt_ram_info_ptr->config_pkt_ram_bank, (pkt_ram_info_ptr->config_base_addr_word)+curr_addr);
                            }
                            else
                            {
                            ...
                            /* Generate data in local struct and write to PKT RAM*/
                            XCVR_LCL_MakeCommonHeader(step_hdr_ptr, *channel_list_ptr,0U, *step_cfo_ptr, *hpm_cal_list_ptr, *phase_add_ptr, *step_cfg_list_ptr);
                            XCVR_LCL_WriteCommonHeader((COM_MODE_CFG_HDR_UNION_Type *)&(mode2_step_ptr->header), step_hdr_ptr);
#else
...
xcvrLclStatus_t XCVR_LCL_ResetCfgResPointers(void)
{
    xcvrLclStatus_t status = gXcvrLclStatusSuccess;
        ...
        uint32_t temp_wrap = ((XCVR_MISC->RSM_CONFIG_BUFF&XCVR_MISC_RSM_CONFIG_BUFF_RSM_CONFIG_DEPTH_MASK)>>XCVR_MISC_RSM_CONFIG_BUFF_RSM_CONFIG_DEPTH_SHIFT);
        ...
        temp_wrap = ((XCVR_MISC->RSM_RESULT_BUFF&XCVR_MISC_RSM_RESULT_BUFF_RSM_RESULT_DEPTH_MASK)>>XCVR_MISC_RSM_RESULT_BUFF_RSM_RESULT_DEPTH_SHIFT);
        (void)temp_wrap;
...
xcvrLclStatus_t XCVR_LCL_LoadConfigSteps(uint32_t ** current_cfg_ptr)
{
    ...
    uint16_t config_index = (temp_config_ptr_reg&XCVR_MISC_RSM_CONFIG_PTR_RSM_CONFIG_WR_PTR_MASK)>>XCVR_MISC_RSM_CONFIG_PTR_RSM_CONFIG_WR_PTR_SHIFT;
    uint8_t config_page = (temp_config_ptr_reg&XCVR_MISC_RSM_CONFIG_PTR_RSM_CONFIG_WR_PAGE_MASK)>>XCVR_MISC_RSM_CONFIG_PTR_RSM_CONFIG_WR_PAGE_SHIFT;
    ...
    for (i=0U; i<count; i++)
    {
        ...
        uint8_t mode = (step_data&COM_MODE_013_CFG_HDR_STEP_CFG_MODE_MASK)>>COM_MODE_013_CFG_HDR_STEP_CFG_MODE_SHIFT;
...
xcvrLclStatus_t XCVR_LCL_ReadResultSteps(uint32_t ** current_res_ptr)
{
    ...
    uint16_t result_index = (temp_result_ptr_reg&XCVR_MISC_RSM_RESULT_PTR_RSM_RESULT_RD_PTR_MASK)>>XCVR_MISC_RSM_RESULT_PTR_RSM_RESULT_RD_PTR_SHIFT;
    uint8_t result_page = (temp_result_ptr_reg&XCVR_MISC_RSM_RESULT_PTR_RSM_RESULT_RD_PAGE_MASK)>>XCVR_MISC_RSM_RESULT_PTR_RSM_RESULT_RD_PAGE_SHIFT;
    ...
    for (i=0U; i<count; i++)
    {
        ...
        {
            // reset PKT RAM pointer to base of buffer
            result_index = ((XCVR_MISC->RSM_RESULT_BUFF&XCVR_MISC_RSM_RESULT_BUFF_RSM_RESULT_BASE_ADDR_MASK)>>XCVR_MISC_RSM_RESULT_BUFF_RSM_RESULT_BASE_ADDR_SHIFT);
        ...
        uint8_t length = (step_data&COM_RES_HDR_SIZE_AGC_IDX_RESULT_SIZE_MASK)>>COM_RES_HDR_SIZE_AGC_IDX_RESULT_SIZE_SHIFT;
#if defined(DEBUG_CIRCULAR_BUFF) && (DEBUG_CIRCULAR_BUFF == 1)
...
xcvrLclStatus_t XCVR_LCL_SetupInitialConfigs(uint8_t total_num_steps, uint32_t * config_in_ptr, uint32_t * results_out_ptr, XCVR_RSM_RTT_TYPE_T rtt_type, uint8_t num_ap, bool sniffer_mode)
{
    ...
    {
        status |= XCVR_LCL_CalcConfigResult_Size((XCVR_RSM_FSTEP_TYPE_T)i, rtt_type, num_ap, &cfg_step_length_words[i], &res_step_length_words[i]);
        if (sniffer_mode)
        {
