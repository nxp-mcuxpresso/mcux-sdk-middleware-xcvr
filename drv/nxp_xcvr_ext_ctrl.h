...
typedef struct
{
    ...
    uint8_t
        tx_rx_switch_pins_enable; /*!< control whether TX and RX_SWITCH pads should be active (0=Disabled, 1=Enabled) */
    ...
    XCVR_FAD_NOT_GPIO_MODE_T
        ant_a_pad_control; /*!< control whether ANT_A pad should be controlled by FAD state machine or TSM GPIO  */
    XCVR_FAD_NOT_GPIO_MODE_T
        ant_b_pad_control; /*!< control whether ANT_B pad should be controlled by FAD state machine or TSM GPIO  */
    XCVR_FAD_NOT_GPIO_MODE_T tx_switch_pad_control; /*!< control whether TX_SWITCH pad should be controlled by FAD state
                                                       machine or TSM GPIO  */
    XCVR_FAD_NOT_GPIO_MODE_T rx_switch_pad_control; /*!< control whether RX_SWITCH pad should be controlled by FAD state
                                                       machine or TSM GPIO  */
    uint8_t pa_tx_wu; /*!< Number of usec to start external PA warmup ahead of internal PA warmup (ramp) start  */
    uint8_t
        pa_tx_wd; /*!< Number of usec to start external PA warmdown ahead of internal PA warmdown (ramp) completion  */
    ...
    XCVR_RX_TX_POLARITY_MODE_T tx_switch_pol_control; /*!< control whether TX_SWITCH pad should be active high or low
                                                         (0=Active HIGH, 1=Active LOW) */
    XCVR_RX_TX_POLARITY_MODE_T rx_switch_pol_control; /*!< control whether RX_SWITCH pad should be active high or low
                                                         (0=Active HIGH, 1=Active LOW) */
...
/*!< T_PM_SLOT values supported. These values are used to set the phase measurement time for a single antenna slot. Enum
 * values are intended to be cast to uint8_t for calculations. */
typedef enum
{
...
/*! @brief Selections for RF_NOT_ALLOWED link layer enable bits. These bits are defined to be OR'd together to use in
 * the RF_NOT_ALLOWED_EN bitfield */
typedef enum
{
...
typedef struct
{
    ...
    uint8_t link_layer_rfna_select; /*!< Enable bits for RF_NOT_ALLOWED signal to LLs. Value is the OR'd together of any
                                       ::coexRfNotAllowLL_t entries.  */
} coexRfNotAllowedConfig_t;

/*! @brief RF_ACTIVE TSM controlled use case configuration structure*/
typedef struct
{
    uint8_t rf_act_extend;     /*!< RF_ACTIVE remains asserted this many microseconds after the end of TSM RX or TX
                                  sequences. Must be <=255. */
    uint8_t rf_act_tx_advance; /*!< RF_ACTIVE aserts this many microseconds before the end of TX warm up. If this value
                                  is > than the number of microseconds before end of TX warm  ramp up in the TSM timing
                                  sequence then an error will be returned. */
    uint8_t rf_act_rx_advance; /*!< RF_ACTIVE aserts this many microseconds before the RX digital is enabled. If this
                                  value is > than the number of microseconds before end of RX warm  ramp up in the TSM
                                  timing sequence then an error will be returned. */
} coexRfActiveTsmConfig_t;

/*! @brief RF_ACTIVE RFMC controlled use case configuration structure*/
typedef struct
{
    bool deassert_when_tsm_idle; /*!< When set to true, RFMS will deassert RF_ACTIVE if the TSM is idle. Otherwise
                                    deassertion is on next low power entry. */
    uint8_t wakeup_delay; /*!< RF_ACTIVE asserts this many 32KHz ref. clocks after the XO is enabled.  Must be less
                             <= 63. */
...
typedef struct
{
    uint8_t rf_stat_tx_advance; /*!< RF_STATUS aserts this many microseconds before the end of TX warm up. If this value
                                   is > than the number of microseconds before end of TX warm  ramp up in the TSM timing
                                   sequence then an error will be returned. */
...
typedef struct
{
    uint8_t rf_pri_tx_advance; /*!< RF_PRIORITY aserts this many microseconds before the end of TX warm up. If this
                                  value is > than the number of microseconds before end of TX warm  ramp up in the TSM
                                  timing sequence then an error will be returned. */
    uint8_t rf_pri_rx_advance; /*!< RF_PRIORITY aserts this many microseconds before the RX digital is enabled. If this
                                  value is > than the number of microseconds before end of RX warm  ramp up in the TSM
                                  timing sequence then an error will be returned. */
 ...
 *
 * @param tsm_controls_coex selects whether the TSM or LL blocks control coexistence. True indicates TSM control, false
 * indicates LL control.
 * @param rf_active_src selects the module to control RF_ACTIVE.
 * @param config_ptr the pointer to a settings structure for setting the signal inversion controls. NULLPTR is a valid
 * input and is used to indicate that no changes should be made to the signal inversion state.
 *
 * @return The status of the selection process.
 *
 * @warning The operation of TSM controlled coexistence signalling will not function properly during
 * Localization/Ranging operations using the Ranging Sequence Manager (RSM). Contol should be given to the LL signals or
 * the coexistence signals should be overridden by software.
 */
 ...
 *
 * This function overrides the state of the RF_NOT_ALLOWED coexistence signal. It allows software to force the radio
 * into an RF not allowed state.
 *
 ...
 *
 * This function reads  the RF_NOT_ALLOWED coexistence signal, either from the raw state indication or a latched state
 * bit.
 *
 * @param read_latched_status When true, the state of RF_NOT_ALLOWED is read from a latched state bit. When false, the
 * state is read from the raw state bit.
 *
 ...
 *
 * This function initializes the RF_ACTIVE coexistence signal for control via TSM (instead of via RFMC or the LL
 * signals).
 *
 ...
 *
 * This function initializes the RF_ACTIVE coexistence signal for control via RFMC (instead of via TSM or the LL
 * signals).
 *
 * @param config_ptr the pointer to a settings structure for RF_ACTIVE initialization.
 *
 ...
 *
 * This function overrides the state of the RF_ACTIVE coexistence signal. It allows software to force the state of the
 * RF activity indication.
 *
 ...
 *
 * @note The amount of timing advance for both TX and RX is limited to the respective TX and RX end of sequence values
 * in the TSM->END_OF_SEQ register. That is, the TSM cannot assert the RF_PRIORITY signal before the TSM is even
 * triggered by the LL to start counting.
 *
 */
 ...
 *
 * This function overrides the state of the RF_STATUS &  RF_PRIORITY coexistence signals. It allows software to force
 * the state of either or both signals.
 *
 ...
 *
 * This function clears all local state variables that control coexistence signals. It is intended to support easy reset
 * of the state for testing.
 *
 */
 ...
 *
 * This function initializes the PA and FEM control register and timing registers before TX or RX operations using those
 * modules.
 *
 ...
 *
 * This function resets the PA and FEM control register and timing registers to allow them to be used for other purposes
 * if required.
 *
 * @return The status of the init process.
 *
 ...
 *
 * This function initializes the PA and FEM control register and timing registers before TX or RX operations using those
 * modules. It sets up to use one or both of the RF_STATUS and RF_PRIORITY pins for PA/FEM control instead of
 * coexistence.
 *
 ...
 * @note
 *  This function is intended to be used when ::XCVR_ExternalFadPaFemInit() cannot be used (if preservation of debugger
 * connection is required. This function does NOT perform any pin muxing settings, that must be performed by a higher
 * layer of software.
 *
 */
 ...
 * @param[in] Antenna_IDs Switching pattern to apply (table where antenna 0 is specified as 0, antenna 1 as 1 and so
 * on...). Must contain as many entries as specified by Switching_Pattern_Length
