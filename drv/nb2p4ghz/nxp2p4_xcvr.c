...
void XCVR_FastPeriphReg_UpDownload_Go(pkt_ram_bank_sel_t pkt_ram_sel, uint32_t offset_in_pkt_ram, bool upload)
{
    ...
    RADIO_CTRL->PACKET_RAM_TO_IPS_CTRL = RADIO_CTRL_PACKET_RAM_TO_IPS_CTRL_PR2IPS_ENA_MASK | 
                                    RADIO_CTRL_PACKET_RAM_TO_IPS_CTRL_PR2IPS_RAM_SEL((uint32_t)pkt_ram_sel) |
...
xcvrStatus_t XCVR_FastPeriphDescrip_Load(uint32_t * descriptor_list, uint16_t num_descrips, volatile uint32_t * pkt_ram_mem_ptr)
{
    ...
    /* For num_descrips */
    /*      Read a descriptor, validate it, and write it to PKT_RAM */
    /*      If the next step will go off the end of PKT_RAM block then set failure status and break out. */
    /*      From the descriptor, take the number of words and put in zeros of that many words. Calculate the next descriptor address.*/
    /* Put a word of zeros to end the list (if not failure status). */
//    if (status == gXcvrSuccess_c)
    else
    {
        uint16_t i;
