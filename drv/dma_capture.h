...
#if !defined(_KW45B41Z82_NBU_H_) && !defined(_KW45B41Z83_NBU_H_) /* DSB IRQ name is different on NBU core */
#define DMA_IRQ DSB_IRQn
#else
#define DMA_IRQ EXT_CPU_INT12_IRQn
#endif
...
typedef enum
{
    ...
    DMA_CAPTURE_NOT_COMPLETE =
        7, /*!< Not an error response, but an indication that capture isn't complete for status polling. */
...
typedef enum
{
    ...
    START_DMA_ON_ZBDEMOD_PREAMBLE_FOUND = 3, /*!< ZBDEMOD Preamble Found Start Trigger.  */
    START_DMA_ON_ZBDEMOD_SFD_MATCH      = 4, /*!< ZBDEMOD SFD Match Start Trigger.  */
#else
    ...
    START_DMA_ON_RBME_CRC_CMPLT = 9, /*!< CRC Processing Complete Start Trigger (asserted when CRC processing completes,
...
#if defined(NXP_RADIO_GEN) && (NXP_RADIO_GEN >= 450)
    START_DMA_ON_RSM_TRIGGER = 12, /*!< RSM Module Triggers DMA capture.  */
#endif
    INVALID_DMA_START_TRIG /*!< Upper limit of DMA start trigger setting, used for error checking. */
...
typedef enum
{
    ...
    DMA_PAGE_RXDIGIQ,     /*!< RXDIG I and Q sample capture. Data is MSB aligned in 16bit portions of the 32bit word. */
    DMA_PAGE_RXDIGIQ_ALT, /*!< RXDIG I and Q sample capture plus additional signals in unused bits. Data is MSB aligned
                             in 16bit portions of the 32bit word. */
    DMA_PAGE_RAWADCIQ, /*!< Raw ADC capture of I and Q channel. Newest bit is in the MS bit of each byte, and newest I/Q
                          samples are in the upper half-word.*/
#if defined(RADIO_IS_GEN_3P5)
    DMA_PAGE_DCESTIQ, /*!< I and Q DCOC tracking estimate. Data is MSB aligned in each 16bit half-word.. */
#endif
    DMA_PAGE_PHASE,      /*!< Wide-band phase, narrowband phase, or high-precision phase (as selected in RXDIG). Newest
                            sample in most significant byte. Each byte contains a MSB-aligned phase sample.*/
    DMA_PAGE_RSSI_PHASE, /*!< 8bit RSSI with 8bit high-resolution phase. Newest sample in most significant half-word.*/
    DMA_PAGE_MAG_PHASE,  /*!< RSSI magnitude with 8bit high-resolution phase.*/
    DMA_PAGE_PHY, /*!< As selected via programming of the PHY, various signals can be output on this 32bit bus for DMA
                     capture. */
    DMA_PAGE_DETERMINISTIC, /*!< Mimics the RXDIGIQ page, but uses deterministic data for software debug purposes. The Q
                               up-counter start from 0x800, the I up-counter starts from 0x000. */
    DMA_PAGE_MAX,           /*!< Upper limit of DMA page setting, used for error checking. */
} dmaPageType_t;
...
typedef enum
{
    ...
    DMA_OUT_DC_RESID_IQ_CORR    = 10, /*!< DC Residual IQ correction output */
    DMA_OUT_INVALID_SEL         = 11, /*!< Invalid output */
} dmaOutputSel_t;
...
#if defined(NXP_RADIO_GEN) && (NXP_RADIO_GEN >= 450)
    DMA_OUT_FRAC_CORR_SEL = 6,    /*!< Frac Corr I/Q output */
    DMA_OUT_DC_RESID_IQ_CORR = 7, /*!< DC Residual IQ correction output */
    DMA_OUT_INVALID_SEL = 8,      /*!< Invalid output*/
#else
...
typedef struct
{
    ...
    dmaStartTriggerType_t
        start_trig;       /*!< Start DMA trigger to be used. Refer to dmaStartTriggerType_t for more information */
...
typedef struct
{
    uint32_t *result_buffer; /*!< Output buffer pointer to store the data; DSB transfers only 32bit chunks  */
